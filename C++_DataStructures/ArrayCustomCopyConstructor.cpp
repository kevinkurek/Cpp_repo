...
class IntArray{
  ...
  public:
    ...
    // allows for deep copy of arrays, so we don't get a double destructor bug by deleting the same memory location with IntArray b = a, because pointer will still delete a's original memory location.
